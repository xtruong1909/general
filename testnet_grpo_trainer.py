...
class TestnetGRPOTrainer(HivemindGRPOTrainer):
    ...
    def train_stages(self, round_num, start_stage, is_coordinator):
        ...
        self.submit_winners(round_num, self.round_winner_fn())

    def _train(self):
        self.follower_train()

    def round_winner_fn(self) -> Sequence[str]:
        my_id = "your_id_here"
        all_candidates = self.stage_data.get_candidates()
        return [my_id] if my_id in all_candidates else []
